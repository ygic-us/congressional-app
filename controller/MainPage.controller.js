sap.ui.define([
	...
	return Controller.extend("generic.timelogger.controller.MainPage", {		
		...
		graph: function(oEvent)
		{
			...
			this.byId("idSummaryHours").removeAllItems();
			...
			var modelData = oStorage.get("myLocalData")
			var array = JSON.parse(modelData).Entries;
			//group by these
			var result = [];
			array.reduce(function(res, value) {
			if (!res[value.Category]) {
				res[value.Category] = { Category: value.Category, TotalTimeWorkedInHours: 0 };
				result.push(res[value.Category])
			}
			res[value.Category].TotalTimeWorkedInHours += parseFloat(value.TotalTimeWorkedInHours);
			return res;
			}, {});
			//converting in to desired form
			var data = result.reduce(function(acc, cur, i) {
			...
			{
				this.byId("idSummaryHours").addItem(new sap.m.DisplayListItem({label: result[i].Category , value:result[i].TotalTimeWorkedInHours +" H"}))
			}
			...
			var pie = d3.pie()
			.value(function(d) {
				return d.value.TotalTimeWorkedInHours;
			})
